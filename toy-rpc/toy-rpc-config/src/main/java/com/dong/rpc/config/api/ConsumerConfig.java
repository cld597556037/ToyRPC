...
public class ConsumerConfig<T> {
    ...
    private void init() {
        ...
        try {
            ...
            //Singleton
            HaStrategy haStrategy = ExtensionLoader.getExtensionLoader(HaStrategy.class)
                    .getExtension(this.protocolConfig.getHaStrategy());
            cluster.setHaStrategy(haStrategy);
            //Prototype
            LoadBalance<T> loadBalance = ExtensionLoader.getExtensionLoader(LoadBalance.class)
                    .getExtension(this.protocolConfig.getLoadBalance());
            cluster.setLoadBalance(loadBalance);
            loadBalance.setCluster(cluster);
    ...
    public void destroy() throws Exception{
        discoveryConfig.getServiceDisovery().unsubscribe(interfaceClass, cluster);
    }
}
