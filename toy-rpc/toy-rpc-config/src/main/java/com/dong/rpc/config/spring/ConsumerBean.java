...
public class ConsumerBean<T> extends ConsumerConfig<T> implements FactoryBean<T>, InitializingBean, DisposableBean, ApplicationContextAware, BeanNameAware {
    ...
    public void afterPropertiesSet() throws Exception {
        if (getProtocolConfig() == null) {
            ...
            logger.info("ConsumerBean 初始化 protocolConfig");
            if (map.size() == 0) {
                protocolConfig = new ProtocolConfig();
                Map<String, DiscoveryConfig> discoveryConfigMap = applicationContext.getBeansOfType(DiscoveryConfig.class, false, false);
                if (discoveryConfigMap.size() == 0) {
                    logger.error("could not find any instance of Class DiscoveryConfig ");
                } else {
                    this.setDiscoveryConfig(discoveryConfigMap.entrySet().iterator().next().getValue());
                }
            } else {
